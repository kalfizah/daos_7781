...
from duns_utils import uns_path
...
class DataMoverTestBase(IorTestBase, MdtestBase):
    ...
    def set_dm_params(self, src_path=None, dst_path=None, dst_pool=None):
        """Set the params for self.tool.

        Args:
            src_path (str, optional): src daos://pool/cont/[path] or POSIX path.
                Default is None, which is ignored.
            dst_path (str, optional): dst daos://pool/cont/[path] or POSIX path
                Default is None, which is ignored.
            dst_pool (TestPool/str, optional): dst pool or uuid. Only used with serialization.
                Default is None, which is ignored.
        """
        if self.tool == "DCP":
            self._set_dcp_params(src_path, dst_path)
        elif self.tool == "DSYNC":
            self._set_dsync_params(src_path, dst_path)
        elif self.tool == "DSERIAL":
            self._set_dserial_params(src_path, dst_pool)
        elif self.tool == "FS_COPY":
            self._set_fs_copy_params(src_path, dst_path)
        elif self.tool == "CONT_CLONE":
            self._set_cont_clone_params(src_path, dst_path)
        else:
    ...
    def set_datamover_params(self,
        ...
        if src_type == "POSIX" or (src_pool is None and src_cont is None):
            _src_path = src_path
        elif src_type in ("DAOS", "DAOS_UUID") or (src_pool is not None and src_cont is not None):
            _src_path = uns_path(src_pool, src_cont, src_path)
        elif src_type == "DAOS_UNS" or (src_cont is not None):
            _src_path = src_cont.path.value.rstrip('/') + src_path

        if dst_type == "POSIX" or (dst_pool is None and dst_cont is None):
            _dst_path = dst_path
        elif dst_type in ("DAOS", "DAOS_UUID") or (dst_pool is not None and dst_cont is not None):
            _dst_path = uns_path(dst_pool, dst_cont, dst_path)
        elif dst_type == "DAOS_UNS" or (dst_cont is not None):
            _dst_path = dst_cont.path.value.rstrip('/') + dst_path

        self.set_dm_params(_src_path, _dst_path, dst_pool)

    def _set_dcp_params(self, src_path=None, dst_path=None):
        ...
        if src_path is not None:
            self.dcp_cmd.set_params(src_path=src_path)

        if dst_path is not None:
            self.dcp_cmd.set_params(dst_path=dst_path)

    def _set_dsync_params(self, src_path=None, dst_path=None):
        ...
        if src_path is not None:
            self.dsync_cmd.set_params(src_path=src_path)

        if dst_path is not None:
            self.dsync_cmd.set_params(dst_path=dst_path)

    def _set_fs_copy_params(self, src_path=None, dst_path=None):
        ...
        if src_path is not None:
            self.fs_copy_cmd.set_fs_copy_params(src=src_path)

        if dst_path is not None:
            self.fs_copy_cmd.set_fs_copy_params(dst=dst_path)

    def _set_cont_clone_params(self, src_path=None, dst_path=None):
        """Set the params for daos cont clone.

        This only supports DAOS -> DAOS copies.

        Args:
            src_path (str): source cont path.
            dst_path (str): destination cont path.
        ...
        if src_path is not None:
            self.cont_clone_cmd.set_cont_clone_params(src=src_path)

        # Set the destination params
        if dst_path is not None:
            self.cont_clone_cmd.set_cont_clone_params(dst=dst_path)

    def _set_dserial_params(self, src_path=None, dst_pool=None):
        ...
        if src_path is not None:
            self.dserialize_cmd.set_params(src_path=src_path, output_path=tmp_path)

        # Set the destination params for ddeserialize
        if dst_pool is not None:
            self.ddeserialize_cmd.set_params(src_path=tmp_path, pool=uuid_from_obj(dst_pool))
    ...
    def run_dm_activities_with_ior(self, tool, create_dataset=False, pool=None, cont=None):
        ...
        if tool == 'CONT_CLONE':
            ...
            self.run_datamover(
                ...
                src_path=uns_path(pool, cont),
                dst_path=uns_path(pool, read_back_cont))
            read_back_pool = pool
        elif tool == 'DSERIAL':
            # Create pool2
            pool2 = self.get_pool()
